...
public class DBResultsetPrint {
  ...
  private static void addColumns(int columnCount, ResultSetMetaData rsmd) throws SQLException {
    ...
    for (int i = 1; i <= columnCount; i++) {
        ...
        // NEW
        /*
          rsmd.getTableName(i) does not always work
        */
        String t = null;
        try {
          t = rsmd.getTableName(i);
        } catch(Exception e) {
          if(sql != null) {
            logger.info("Using regex to find tbl name");
            ...
            if (m.find())
            {
              ...
              t = m.group(1);
              logger.info("Tbl name: " + t);
            } else
              logger.error("Using regex to find tbl name failed");
          }
          else {
            logger.error("Find tbl name failed");
            t = "?";
          }
        }
